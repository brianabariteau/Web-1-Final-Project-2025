...
// * Chosen Characters Panel
const chosenPanel = $("<div>").addClass("characters-chosen");

if (mode === "single player") {
  player1Character = $("<p>").text(`Player 1: -`).addClass("p1");

  // * Choose Character | Player 1
  const player1 = $("<button>").addClass("player-1").text("Player 1");

  $(player1).on("click", () => {
    currentPlayer = "p1";
  });

  $(chosenPanel).append(player1, player1Character);
} else if (mode === "multiplayer") {
  player1Character = $("<p>").text(`Player 1: -`).addClass("p1");
  player2Character = $("<p>").text(`Player 2: -`).addClass("p2");
  ...
  // * Choose Character | Player 2
  const player2 = $("<button>").addClass("player-2").text("Player 2");

  $(player2).on("click", () => {
    currentPlayer = "p2";
  });

  $(chosenPanel).append(player1, player2, player1Character, player2Character);
}
...
function selectCharacter(avatarElement, character) {
  if (currentPlayer === "p1") {
    ...
    player1Character.text(`Player 1: ${character.name}`);
    ...
    player2Character.text(`Player 2: ${character.name}`);
  }
}

// * Text Box + Buttons
const bottomPanel = $("<div>").addClass("bottom-panel");
const textBox = $("<p>").addClass("box");
...
if (mode === "single player") {
  ...
  textBox.text("Please choose a character.");
}
...
$(confirmButton).on("click", () => {
  if (mode === "single player") {
    if (!player1Selected) {
      alert("Player 1, please select a character first!");
      return;
    }
  } else if (mode === "multiplayer") {
    if (!player1Selected && !player2Selected) {
      alert("Player 1 and Player 2, please select a character first!");
      return;
    } else if (!player1Selected) {
      alert("Player 1, please select a character first!");
      return;
    } else if (!player2Selected) {
      alert("Player 2, please select a character first!");
      return;
    }
  }

  localStorage.setItem("player1Character", JSON.stringify(player1Selected));
  localStorage.setItem("player2Character", JSON.stringify(player2Selected));
...
$(cancelButton).on("click", () => {
  if (mode === "single player") {
    ...
    player1Character = $("<p>").text(`Player 1: -`);
    selectedPlayer = null;
    ...
    if (currentPlayer === "p1") {
      player1Selected = null;
      player1Character.text(`Player 1: -`);
    } else if (currentPlayer === "p2") {
      player2Selected = null;
      player2Character.text(`Player 2: -`);
    }
...
$("body").append(background, switchCharacters, bottomPanel, chosenPanel);
