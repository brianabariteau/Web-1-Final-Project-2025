import { CUSTOM_STATS } from "./characters.js";

export const CHARACTER_MOVES = {
  Zeus: {
    move1name: "Thunderbolt Wrath",
    move1type: "Damage",
    move2name: "Skybreaker Slam",
    move2type: "Power",
    move3name: "Stormcaller Command",
    move3type: "Buff",
    move4name: "God of Thunder",
    move4type: "Ultimate",
    move1desc: "Strike the enemy with lightning, dealing heavy damage.",
    move2desc: "A powerful thunder crash that deals increased damage.",
    move3desc:
      "Strike the enemy while empowering yourself, increasing Power and Speed.",
    move4desc: "Unleash a devastating lightning strike.",

    move1use(attacker, defender) {
      const damage =
        damageDealt(attacker, defender, 1.5) +
        (attacker.stats.power - defender.stats.power);
      defender.stats.currenthp = Math.max(0, defender.stats.currenthp - damage);

      // ! Future stuff, intended idea, didn't implement
      defender.status.push({ type: "shock", turns: 1 });

      return { damage: damage, status: "shock", defender: defender.status };
    },
    move2use(attacker, defender) {
      const damage = damageDealt(attacker, defender, 1.5);
      defender.stats.currenthp = Math.max(0, defender.stats.currenthp - damage);

      return { damage: damage, ignoreDefense: true };
    },
    move3use(attacker, defender) {
      // ! Future stuff, intended idea
      //attacker.buffs.push(statusApplied("buffs", "power", 20, 2));
      //attacker.buffs.push(statusApplied("buffs", "speed", 20, 2));
      //return { buff: ["power", "speed"] };

      // ? temporary
      const damage = damageDealt(attacker, defender, 1.2);
      defender.stats.currenthp = Math.max(0, defender.stats.currenthp - damage);
      attacker.stats.power += 20;
      attacker.stats.speed += 20;

      return {
        damage: damage,
        power: attacker.stats.power,
        speed: attacker.stats.speed,
      };
    },
    move4use(attacker, defender) {
      const damage = damageDealt(attacker, defender, 2.2);
      defender.stats.currenthp = Math.max(0, defender.stats.currenthp - damage);

      // ! Future stuff, intended idea, didn't implement
      //defender.status.push({ type: "paralyze", turns: 1 });

      return { damage: damage, status: "paralyze" };
    },
    ability1name: CUSTOM_STATS["Zeus"].domainMastery,
    ability1desc: CUSTOM_STATS["Zeus"].masteryDesc,
    ability2name: CUSTOM_STATS["Zeus"].specialTrait,
    ability2desc: CUSTOM_STATS["Zeus"].traitDesc,
    ability1use(character) {
      character.stats.power *= 0.25;
    },
    ability2use(character, opponent) {
      opponent.stats.currenthp = Math.max(0, opponent.stats.currenthp * 0.1);
    },
  },
  Athena: {
    move1name: "Spear of Wisdom",
    move1type: "Damage",
    move2name: "Aegis Wall",
    move2type: "Buff",
    move3name: "Tactical Feint",
    move3type: "Control",
    move4name: "Perfect Counter",
    move4type: "Ultimate",
    move1desc: "A precise attack that scales with Athena's Defense.",
    move2desc: "Raise your Defense significantly.",
    move3desc: "A tactical strike.",
    move4desc: "Counter with overwhelming force based on the enemy's strength.",
    move1use(attacker, defender) {
      const damage =
        damageDealt(attacker, defender, 1.0) +
        Math.floor(attacker.stats.defense * 0.3);

      defender.stats.currenthp = Math.max(0, defender.stats.currenthp - damage);

      return { damage: damage };
    },
    move2use(attacker, defender) {
      // ! Future stuff, intended idea
      //attacker.buffs.push(statusApplied("buff", "defense", 30, 2));
      //return { buff: ["defense"] };

      attacker.stats.defense += 30;

      return { defense: attacker.stats.defense };
    },
    move3use(attacker, defender) {
      // ! Future stuff, indended idea
      //defender.status.push({ type: "delay", turns: 1 });
      //return { status: "delay" };

      // ? temporary
      const damage = damageDealt(attacker, defender, 1.2);
      defender.stats.currenthp = Math.max(0, defender.stats.currenthp - damage);

      return { damage: damage };
    },
    move4use(attacker, defender) {
      // ! Future stuff, intended idea
      //attacker.buffs.push(statusApplied("buff", "damageReduction", 40, 3));

      // ? temporary
      const damage =
        Math.floor(CUSTOM_STATS[defender.name].power) +
        damageDealt(attacker, defender, 1.0);

      defender.stats.currenthp = Math.max(0, defender.stats.currenthp - damage);

      return { damage: damage };
    },
    ability1name: CUSTOM_STATS["Athena"].domainMastery,
    ability1desc: CUSTOM_STATS["Athena"].masteryDesc,
    ability2name: CUSTOM_STATS["Athena"].specialTrait,
    ability2desc: CUSTOM_STATS["Athena"].traitDesc,
    ability1use(character) {
      character.stats.defense *= 0.1;
    },
    ability2use(character, opponent) {
      opponent.stats.attack *= 0.3;
    },
  },
  Hestia: {
    move1name: "Hearthfire Lash",
    move1type: "Damage",
    move2name: "Sanctuary Flame",
    move2type: "Heal",
    move3name: "Comforting Warmth",
    move3type: "Buff",
    move4name: "Eternal Hearth",
    move4type: "Ultimate",
    move1desc: "A gentle but steady flame attack.",
    move2desc: "Restore your own health.",
    move3desc: "Bolster resilience and stamina, then strike the enemy.",
    move4desc: "Restore a large amount of health through eternal flame.",
    move1use(attacker, defender) {
      const damage = damageDealt(attacker, defender, 0.9);
      defender.stats.currenthp = Math.max(0, defender.stats.currenthp - damage);

      // ! Future stuff, intended idea, not implemented
      //defender.status.push({ type: "burn", turns: 2 });

      return { damage: damage };
    },
    move2use(attacker, defender) {
      const health = heal(attacker, 0.6);
      attacker.stats.currenthp = Math.min(
        attacker.maxhp,
        attacker.stats.currenthp + health
      );

      return {
        health: health,
        hp: attacker.stats.currenthp,
        maxhp: attacker.maxhp,
      };
    },
    move3use(attacker, defender) {
      // ! Future stuff, intended idea, not implemented
      //attacker.buffs.push(statusApplied("buff", "resilience", 20, 2));
      //attacker.buffs.push(statusApplied("buff", "stamina", 20, 2));

      attacker.stats.resilience += 20;
      attacker.stats.stamina += 5;

      // ? temporary
      const damage =
        damageDealt(attacker, defender, 0.5) + attacker.stats.stamina;

      defender.stats.currenthp = Math.max(0, defender.stats.currenthp - damage);

      return {
        damage: damage,
        resilience: attacker.stats.resilience,
        stamina: attacker.stats.stamina,
      };
    },
    move4use(attacker, defender) {
      const health = heal(attacker, 1.0);
      attacker.stats.currenthp = Math.min(
        attacker.maxhp,
        attacker.stats.currenthp + health
      );

      // ! Future stuff, intended idea, not implemented
      //attacker.status.push({ type: "burn-aura", turns: 3 });

      return { health: health, hp: attacker.stats.currenthp };
    },
    ability1name: CUSTOM_STATS["Hestia"].domainMastery,
    ability1desc: CUSTOM_STATS["Hestia"].masteryDesc,
    ability2name: CUSTOM_STATS["Hestia"].specialTrait,
    ability2desc: CUSTOM_STATS["Hestia"].traitDesc,
    ability1use(character) {
      character.stats.power *= 0.2;
    },
    ability2use(character) {
      character.stats.currenthp = Math.min(
        character.maxhp,
        character.stats.currenthp + 20
      );
    },
  },
  Apollo: {
    move1name: "Radiant Arrow",
    move1type: "Damage",
    move2name: "Hymn of Clarity",
    move2type: "Buff",
    move3name: "Physician's Blessing",
    move3type: "Heal",
    move4name: "Solar Apothesis",
    move4type: "Ultimate",
    move1desc: "A swift radiant attack fueled by divine power.",
    move2desc: "Increase your Power through divine inspiration.",
    move3desc: "Heal yourself with the regenerative energy of the sun.",
    move4desc: "Unleash overwhelming solar energy in a powerful strike.",
    move1use(attacker, defender) {
      const damage =
        damageDealt(attacker, defender, 0.8) + attacker.stats.power;
      defender.stats.currenthp = Math.max(0, defender.stats.currenthp - damage);

      return { damage: damage };
    },
    move2use(attacker, defender) {
      // ! Future stuff, intended idea, not implemented
      //attacker.buffs.push(statusApplied("buff", "power", 25, 2));

      attacker.stats.power += 15;

      return { power: attacker.stats.power };
    },
    move3use(attacker, defender) {
      const health = heal(attacker, 1.25);
      attacker.stats.currenthp = Math.min(
        attacker.maxhp,
        attacker.stats.currenthp + health
      );

      return { health: health };
    },
    move4use(attacker, defender) {
      const damage = damageDealt(attacker, defender, 2.15);
      defender.stats.currenthp = Math.max(0, defender.stats.currenthp - damage);

      return { damage: damage };
    },
    ability1name: CUSTOM_STATS["Apollo"].domainMastery,
    ability1desc: CUSTOM_STATS["Apollo"].masteryDesc,
    ability2name: CUSTOM_STATS["Apollo"].specialTrait,
    ability2desc: CUSTOM_STATS["Apollo"].traitDesc,
    ability1use(character) {
      character.stats.power *= 0.2;
    },
    ability2use(character) {
      character.stats.attack += 20;
    },
  },
  Artemis: {
    move1name: "Moonlit Shot",
    move1type: "Damage",
    move2name: "Hunter's Sight",
    move2type: "Debuff",
    move3name: "Silent Step",
    move3type: "Buff",
    move4name: "Goddess of the Hunt",
    move4type: "Ultimate",
    move1desc: "A deadly moonlit shot that always lands as a critical hit.",
    move2desc: "Mark the enemy with a fast strike that scales with Speed.",
    move3desc: "Weaken the enemy's defenses through swift movement.",
    move4desc: "Deliver a devastating guaranteed critical strike.",
    move1use(attacker, defender) {
      const damage = damageDealt(attacker, defender, 1.0) * 2;
      defender.stats.currenthp = Math.max(0, defender.stats.currenthp - damage);

      return { damage: damage };
    },
    move2use(attacker, defender) {
      const damage =
        damageDealt(attacker, defender, 0.6) + attacker.stats.speed;
      defender.stats.currenthp = Math.max(0, defender.stats.currenthp - damage);

      return { damage: damage };
    },
    move3use(attacker, defender) {
      // ! Future ideas
      // attacker.buffs.push(statusApplied("buff", "speed", 25, 2));
      // attacker.buffs.push(statusApplied("buff", "evasion", 25, 2));
      defender.stats.defense -= 25;

      return { defense: defender.stats.defense };
    },
    move4use(attacker, defender) {
      const damage = damageDealt(attacker, defender, 3.0);
      defender.stats.currenthp = Math.max(0, defender.stats.currenthp - damage);

      return { damage: damage };
    },
    ability1name: CUSTOM_STATS["Artemis"].domainMastery,
    ability1desc: CUSTOM_STATS["Artemis"].masteryDesc,
    ability2name: CUSTOM_STATS["Artemis"].specialTrait,
    ability2desc: CUSTOM_STATS["Artemis"].traitDesc,
    ability1use(character) {
      character.stats.speed *= 0.25;
    },
    ability2use(character) {
      character.stats.attack += 20;
    },
  },
  Poseidon: {
    move1name: "Tidal Crash",
    move1type: "Damage",
    move2name: "Ocean's Fury",
    move2type: "Power",
    move3name: "Seaquake",
    move3type: "Control",
    move4name: "Trident Supremacy",
    move4type: "Ultimate",
    move1desc: "Crash a powerful tidal wave into the enemy.",
    move2desc: "Strike the enemy and increase your Attack.",
    move3desc: "Shatter defenses with a seaquake, lowering enemy Defense.",
    move4desc: "Overwhelm the enemy with trident-forged water power.",
    move1use(attacker, defender) {
      const damage = damageDealt(attacker, defender, 1.2);
      defender.stats.currenthp = Math.max(0, defender.stats.currenthp - damage);

      return { damage: damage };
    },
    move2use(attacker, defender) {
      const damage = damageDealt(attacker, defender, 1.0);
      defender.stats.currenthp = Math.max(0, defender.stats.currenthp - damage);

      attacker.stats.attack += 30;

      return { damage: damage };
    },
    move3use(attacker, defender) {
      // defender.status.push({ type: "stun", turns: 1 })
      const damage = 30;
      defender.stats.currenthp = Math.max(0, defender.stats.currenthp - damage);

      defender.stats.defense -= 15;

      return { damage: damage };
    },
    move4use(attacker, defender) {
      const damage = damageDealt(attacker, defender, 2.5);
      defender.stats.currenthp = Math.max(0, defender.stats.currenthp - damage);

      return { damage: damage };
    },
    ability1name: CUSTOM_STATS["Poseidon"].domainMastery,
    ability1desc: CUSTOM_STATS["Poseidon"].masteryDesc,
    ability2name: CUSTOM_STATS["Poseidon"].specialTrait,
    ability2desc: CUSTOM_STATS["Poseidon"].traitDesc,
    ability1use(character) {
      character.stats.power *= 0.5;
    },
    ability2use(character) {
      character.stats.attack *= 0.2;
    },
  },
  Hades: {
    move1name: "Stygian Strike",
    move1type: "Damage",
    move2name: "Gravebind Chains",
    move2type: "Control",
    move3name: "Dread Decree",
    move3type: "Debuff",
    move4name: "King of the Underworld",
    move4type: "Ultimate",
    move1desc: "Strike the enemy and drain a portion of their life.",
    move2desc: "Bind the enemy, reducing their Defense and Attack.",
    move3desc: "Crush enemy morale by lowering Power and Resilience.",
    move4desc: "Drain massive life force from the enemy to heal yourself.",
    move1use(attacker, defender) {
      const damage = damageDealt(attacker, defender, 1.0);
      defender.stats.currenthp = Math.max(0, defender.stats.currenthp - damage);

      attacker.stats.currenthp = Math.min(
        attacker.maxhp,
        attacker.stats.currenthp + Math.floor(damage * 0.3)
      );

      return { damage: damage };
    },
    move2use(attacker, defender) {
      //defender.status.push({ type: "root", turns: 1 });

      defender.stats.defense = Math.max(0, defender.stats.defense - 20);
      defender.stats.attack = Math.max(0, defender.stats.attack - 20);

      return { defense: defender.stats.defense };
    },
    move3use(attacker, defender) {
      //defender.debuffs.push(statusApplied("debuff", "power", -20, 2))

      defender.stats.power = Math.max(0, defender.stats.power - 20);
      defender.stats.resilience = Math.max(0, defender.stats.resilience - 50);

      //console.log(`${defender.name} has power of ${defender.stats.power}`);
      //console.log(
      //  `${defender.name} has resilience of ${defender.stats.resilience}`
      //);

      return {
        defense: defender.stats.power,
        resistance: defender.stats.resistance,
      };
    },
    move4use(attacker, defender) {
      const damage = damageDealt(attacker, defender, 1.8);
      defender.stats.currenthp = Math.max(0, defender.stats.currenthp - damage);

      attacker.stats.currenthp += Math.floor(damage * 0.5);

      return { damage: damage };
    },
    ability1name: CUSTOM_STATS["Hades"].domainMastery,
    ability1desc: CUSTOM_STATS["Hades"].masteryDesc,
    ability2name: CUSTOM_STATS["Hades"].specialTrait,
    ability2desc: CUSTOM_STATS["Hades"].traitDesc,
    ability1use(character) {
      character.stats.attack *= 0.3;
    },
    ability2use(character, opponent) {
      const hp = Math.max(0, opponent.stats.currenthp / 0.05);
      opponent.stats.currenthp = Math.max(0, opponent.stats.currenthp / 0.05);
      character.stats.currenthp += hp;
    },
  },
  Demeter: {
    move1name: "Harvest Scythe",
    move1type: "Damage",
    move2name: "Verdant Blessing",
    move2type: "Heal",
    move3name: "Fallow Curse",
    move3type: "Debuff",
    move4name: "Wrath of the Seasons",
    move4type: "Ultimate",
    move1desc: "A natural strike empowered by the land.",
    move2desc: "Restore your health through nature's blessing.",
    move3desc: "Wither enemy strength by reducing their Attack.",
    move4desc: "Invoke the seasons to deal damage and weaken all enemy stats.",
    move1use(attacker, defender) {
      const damage = damageDealt(attacker, defender, 1.2);
      defender.stats.currenthp = Math.max(0, defender.stats.currenthp - damage);

      return { damage: damage };
    },
    move2use(attacker, defender) {
      const health = heal(attacker, 0.8);
      attacker.stats.currenthp = Math.min(
        attacker.maxhp,
        attacker.stats.currenthp + health
      );

      return { health: health };
    },
    move3use(attacker, defender) {
      //defender.debuffs.push(statusApplied("debuff", "attack", -20, 2));
      defender.stats.attack = Math.max(0, defender.stats.attack - 20);

      return {
        defense: defender.stats.defense,
      };
    },
    move4use(attacker, defender) {
      const spring = 2.5;
      const summer = 3;
      const winter = 1.0;
      const fall = 1.5;

      let seasons = [spring, summer, winter, fall];

      const randomSeason = seasons[Math.floor(Math.random() * seasons.length)];

      const damage = damageDealt(attacker, defender, randomSeason);
      defender.stats.currenthp = Math.max(0, defender.stats.currenthp - damage);

      defender.stats.speed = Math.max(0, defender.stats.speed - 10);
      defender.stats.attack = Math.max(0, defender.stats.attack - 10);
      defender.stats.defense = Math.max(0, defender.stats.defense - 10);
      defender.stats.power = Math.max(0, defender.stats.power - 10);

      return {
        damage: damage,
        defense: defender.stats.defense,
        speed: defender.stats.speed,
        attack: defender.stats.attack,
        power: defender.stats.power,
      };
    },
    ability1name: CUSTOM_STATS["Demeter"].domainMastery,
    ability1desc: CUSTOM_STATS["Demeter"].masteryDesc,
    ability2name: CUSTOM_STATS["Demeter"].specialTrait,
    ability2desc: CUSTOM_STATS["Demeter"].traitDesc,
    ability1use(character) {
      character.stats.power *= 0.45;
    },
    ability2use(character, opponent) {
      opponent.stats.attack = Math.max(0, opponent.stats.attack / 0.2);
    },
  },
  Aphrodite: {
    move1name: "Roseblade Slash",
    move1type: "Damage",
    move2name: "Beguiling Smile",
    move2type: "Control",
    move3name: "Swaying Perfume",
    move3type: "Debuff",
    move4name: "Irresistible Aura",
    move4type: "Ultimate",
    move1desc: "A charm-infused slash that deals direct damage.",
    move2desc: "Disorient the enemy with a distracting attack.",
    move3desc: "Reduce the enemy's combat effectiveness.",
    move4desc: "Overwhelm the enemy's senses, leaving them vulnerable.",

    move1use(attacker, defender) {
      const damage =
        damageDealt(attacker, defender, 0.9) + attacker.stats.power;
      defender.stats.currenthp = Math.max(0, defender.stats.currenthp - damage);

      return { damage: damage };
    },
    move2use(attacker, defender) {
      const damage = damageDealt(attacker, defender, 0.6);
      defender.stats.currenthp = Math.max(0, defender.stats.currenthp - damage);

      defender.stats.speed = Math.max(0, defender.stats.speed - 15);

      return { damage, speed: defender.stats.speed };
    },
    move3use(attacker, defender) {
      defender.stats.attack = Math.max(0, defender.stats.attack - 20);
      defender.stats.power = Math.max(0, defender.stats.power - 20);

      attacker.stats.attack += 20;

      return { attack: defender.stats.attack, power: defender.stats.power };
    },
    move4use(attacker, defender) {
      const damage = damageDealt(attacker, defender, 1.6);
      defender.stats.currenthp = Math.max(0, defender.stats.currenthp - damage);

      defender.stats.speed = Math.max(0, defender.stats.speed - 25);

      return { damage: damage };
    },
    ability1name: CUSTOM_STATS["Aphrodite"].domainMastery,
    ability1desc: CUSTOM_STATS["Aphrodite"].masteryDesc,
    ability2name: CUSTOM_STATS["Aphrodite"].specialTrait,
    ability2desc: CUSTOM_STATS["Aphrodite"].traitDesc,
    ability1use(character) {
      character.stats.power *= 0.25;
    },
    ability2use(character) {
      character.stats.defense *= 0.5;
    },
  },
  Hera: {
    move1name: "Queen's Judgment",
    move1type: "Damage",
    move2name: "Royal Edict",
    move2type: "Debuff",
    move3name: "Matron's Guard",
    move3type: "Buff",
    move4name: "Divine Authority",
    move4type: "Ultimate",
    move1desc: "Deliver a punishing divine judgment.",
    move2desc: "Weaken the enemy through royal authority.",
    move3desc: "Strengthen defenses through maternal protection.",
    move4desc: "Assert divine dominance over the battlefield.",
    move1use(attacker, defender) {
      const damage = damageDealt(attacker, defender, 1.3);
      defender.stats.currenthp = Math.max(0, defender.stats.currenthp - damage);

      return { damage: damage };
    },
    move2use(attacker, defender) {
      defender.stats.attack = Math.max(0, defender.stats.attack - 15);
      defender.stats.defense = Math.max(0, defender.stats.defense - 15);
      defender.stats.power = Math.max(0, defender.stats.power - 15);
      defender.stats.speed = Math.max(0, defender.stats.speed - 15);

      return {
        attack: defender.stats.attack,
        defense: defender.stats.defense,
        power: defender.stats.power,
        speed: defender.stats.power,
      };
    },
    move3use(attacker, defender) {
      attacker.stats.defense += 30;
      attacker.stats.resilience += 20;

      return {
        defense: attacker.stats.defense,
        resilience: attacker.stats.resilience,
      };
    },
    move4use(attacker, defender) {
      const damage = damageDealt(attacker, defender, 2.0);
      defender.stats.currenthp = Math.max(0, defender.stats.currenthp - damage);

      return { damage: damage };
    },
    ability1name: CUSTOM_STATS["Hera"].domainMastery,
    ability1desc: CUSTOM_STATS["Hera"].masteryDesc,
    ability2name: CUSTOM_STATS["Hera"].specialTrait,
    ability2desc: CUSTOM_STATS["Hera"].traitDesc,
    ability1use(character) {
      character.stats.power *= 0.2;
    },
    ability2use(character) {
      character.stats.defense *= 0.5;
    },
  },
  Dionysus: {
    move1name: "Vine Lash",
    move1type: "Damage",
    move2name: "Ecstatic Toast",
    move2type: "Buff",
    move3name: "Wine-Drunk Delirium",
    move3type: "Control",
    move4name: "God of Madness",
    move4type: "Ultimate",
    move1desc: "Strike the enemy with writhing vines.",
    move2desc: "Grant yourself a chaotic divine blessing.",
    move3desc:
      "Confuse the enemy with intoxicating madness that lowers their Attack and Speed.",
    move4desc: "Unleash unpredictable divine chaos.",

    move1use(attacker, defender) {
      const damage =
        damageDealt(attacker, defender, 1.0) + attacker.stats.attack;
      defender.stats.currenthp = Math.max(0, defender.stats.currenthp - damage);

      return { damage };
    },
    move2use(attacker, defender) {
      const buffs = ["attack", "speed", "power", "defense", "resilience"];
      const randomBuff = buffs[Math.floor(Math.random() * buffs.length)];
      attacker.stats[randomBuff] += 30;

      return { buff: randomBuff };
    },
    move3use(attacker, defender) {
      defender.stats.attack = Math.max(0, defender.stats.attack - 20);
      defender.stats.speed = Math.max(0, defender.stats.speed - 20);

      return { attack: defender.stats.attack, speed: defender.stats.speed };
    },
    move4use(attacker, defender) {
      const damage = damageDealt(attacker, defender, 1.8);
      defender.stats.currenthp = Math.max(0, defender.stats.currenthp - damage);

      attacker.stats.attack += 15;
      attacker.stats.speed += 15;

      return { damage: damage };
    },
    ability1name: CUSTOM_STATS["Dionysus"].domainMastery,
    ability1desc: CUSTOM_STATS["Dionysus"].masteryDesc,
    ability2name: CUSTOM_STATS["Dionysus"].specialTrait,
    ability2desc: CUSTOM_STATS["Dionysus"].traitDesc,
    ability1use(character) {
      character.stats.power *= 0.2;
    },
    ability2use(character) {
      const buffs = ["attack", "speed", "power", "defense", "resilience"];
      const randomBuff = buffs[Math.floor(Math.random() * buffs.length)];
      character.stats[randomBuff] += 30;
    },
  },
  Ares: {
    move1name: "Warcry Slash",
    move1type: "Damage",
    move2name: "Berserker Rush",
    move2type: "Power",
    move3name: "Bloodied Momentum",
    move3type: "Buff",
    move4name: "Bloodlust Rampage",
    move4type: "Ultimate",
    move1desc: "A brutal strike driven by raw aggression.",
    move2desc: "Rush the enemy, growing stronger as health drops.",
    move3desc: "Increase Attack through blood-soaked momentum.",
    move4desc: "Unleash a relentless flurry of devastating attacks.",

    move1use(attacker, defender) {
      const damage = damageDealt(attacker, defender, 1.3);
      defender.stats.currenthp = Math.max(0, defender.stats.currenthp - damage);

      return { damage: damage };
    },
    move2use(attacker, defender) {
      const missingHp = attacker.maxhp - attacker.stats.currenthp;
      const bonusAttack = Math.floor(missingHp * 0.2);

      const damage = damageDealt(attacker, defender, 1.0) + bonusAttack;
      defender.stats.currenthp = Math.max(0, defender.stats.currenthp - damage);

      return { damage: damage };
    },
    move3use(attacker, defender) {
      attacker.stats.attack += 20;

      return { attack: attacker.stats.attack };
    },
    move4use(attacker, defender) {
      const damage = damageDealt(attacker, defender, 2.8);
      defender.stats.currenthp = Math.max(0, defender.stats.currenthp - damage);

      return { damage };
    },
    ability1name: CUSTOM_STATS["Ares"].domainMastery,
    ability1desc: CUSTOM_STATS["Ares"].masteryDesc,
    ability2name: CUSTOM_STATS["Ares"].specialTrait,
    ability2desc: CUSTOM_STATS["Ares"].traitDesc,
    ability1use(character) {
      character.stats.attack *= 0.2;
    },
    ability2use(character) {
      const missingHp = character.maxhp - character.stats.currenthp;
      const bonusAttack = Math.floor(missingHp * 0.2);

      character.stats.attack += bonusAttack;
    },
  },
  Hephaestus: {
    move1name: "Molten Hammer",
    move1type: "Damage",
    move2name: "Forge Temper",
    move2type: "Buff",
    move3name: "Searing Shackles",
    move3type: "Control",
    move4name: "Masterwork Cataclysm",
    move4type: "Ultimate",
    move1desc: "Smash the enemy with molten force.",
    move2desc: "Temper yourself, increasing Defense and Power.",
    move3desc: "Bind the enemy with searing restraints, lowering their Speed.",
    move4desc: "Detonate a massive forged explosion.",

    move1use(attacker, defender) {
      const damage = damageDealt(attacker, defender, 1.4);
      defender.stats.currenthp = Math.max(0, defender.stats.currenthp - damage);

      return { damage: damage };
    },
    move2use(attacker, defender) {
      attacker.stats.defense += 25;
      attacker.stats.power += 20;

      return {
        defense: attacker.stats.defense,
        power: attacker.stats.power,
      };
    },
    move3use(attacker, defender) {
      defender.stats.speed = Math.max(0, defender.stats.speed - 30);

      return { speed: defender.stats.speed };
    },
    move4use(attacker, defender) {
      const damage = damageDealt(attacker, defender, 2.6);
      defender.stats.currenthp = Math.max(0, defender.stats.currenthp - damage);

      return { damage: damage };
    },
    ability1name: CUSTOM_STATS["Hephaestus"].domainMastery,
    ability1desc: CUSTOM_STATS["Hephaestus"].masteryDesc,
    ability2name: CUSTOM_STATS["Hephaestus"].specialTrait,
    ability2desc: CUSTOM_STATS["Hephaestus"].traitDesc,
    ability1use(character) {
      character.stats.attack *= 0.2;
    },
    ability2use(character, opponent) {
      opponent.stats.attack = 0;
    },
  },
  Persephone: {
    move1name: "Springthorn Strike",
    move1type: "Damage",
    move2name: "Blooming Remedy",
    move2type: "Heal",
    move3name: "Underworld Veil",
    move3type: "Debuff",
    move4name: "Dual Aspect Reign",
    move4type: "Ultimate",
    move1desc: "Strike the enemy while draining their life.",
    move2desc: "Restore health with blooming vitality.",
    move3desc: "Lower the enemy's defenses with underworld power.",
    move4desc:
      "Unleash dual divine authority over life and death, increasing your Power.",

    move1use(attacker, defender) {
      const damage =
        damageDealt(attacker, defender, 1.0) + attacker.stats.power;
      defender.stats.currenthp = Math.max(0, defender.stats.currenthp - damage);

      attacker.stats.currenthp = Math.min(
        attacker.maxhp,
        attacker.stats.currenthp + Math.floor(damage * 0.4)
      );

      return { damage: damage };
    },
    move2use(attacker, defender) {
      const health = heal(attacker, 1.2);
      attacker.stats.currenthp = Math.min(
        attacker.maxhp,
        attacker.stats.currenthp + health
      );

      return { health: health };
    },
    move3use(attacker, defender) {
      defender.stats.defense = Math.max(0, defender.stats.defense - 25);

      return { defense: defender.stats.defense };
    },
    move4use(attacker, defender) {
      const damage = damageDealt(attacker, defender, 2.0);
      defender.stats.currenthp = Math.max(0, defender.stats.currenthp - damage);

      attacker.stats.power += 30;

      return { damage: damage };
    },
    ability1name: CUSTOM_STATS["Persephone"].domainMastery,
    ability1desc: CUSTOM_STATS["Persephone"].masteryDesc,
    ability2name: CUSTOM_STATS["Persephone"].specialTrait,
    ability2desc: CUSTOM_STATS["Persephone"].traitDesc,
    ability1use(character) {
      character.stats.power += 30;
    },
    ability2use(character) {
      character.stats.speed *= 0.2;
      character.stats.attack *= 0.2;
      character.stats.defense *= 0.2;
      character.stats.power *= 0.2;
    },
  },
  Hermes: {
    move1name: "Swift Jab",
    move1type: "Damage",
    move2name: "Trickster's Swap",
    move2type: "Control",
    move3name: "Courier's Blessing",
    move3type: "Buff",
    move4name: "Winged Sandals Blitz",
    move4type: "Ultimate",
    move1desc: "A quick and precise jab.",
    move2desc: "Disrupt the enemy's momentum by lowering their Speed.",
    move3desc: "Increase Speed with divine swiftness.",
    move4desc: "Strike rapidly in a blur of motion.",

    move1use(attacker, defender) {
      const damage =
        damageDealt(attacker, defender, 1.0) + attacker.stats.speed;
      defender.stats.currenthp = Math.max(0, defender.stats.currenthp - damage);

      return { damage: damage };
    },
    move2use(attacker, defender) {
      defender.stats.speed = Math.max(0, defender.stats.speed - 25);

      return { speed: defender.stats.speed };
    },
    move3use(attacker, defender) {
      attacker.stats.speed += 30;

      return { speed: defender.stats.speed };
    },
    move4use(attacker, defender) {
      const damage = damageDealt(attacker, defender, 2.1) * 3;
      defender.stats.currenthp = Math.max(0, defender.stats.currenthp - damage);

      return { damage: damage };
    },
    ability1name: CUSTOM_STATS["Hermes"].domainMastery,
    ability1desc: CUSTOM_STATS["Hermes"].masteryDesc,
    ability2name: CUSTOM_STATS["Hermes"].specialTrait,
    ability2desc: CUSTOM_STATS["Hermes"].traitDesc,
    ability1use(character) {
      character.stats.speed *= 0.2;
    },
    ability2use(character) {
      character.stats.power += 20;
    },
  },
  Nyx: {
    move1name: "Void Talons",
    move1type: "Damage",
    move2name: "Nightfall Shroud",
    move2type: "Debuff",
    move3name: "Starless Silence",
    move3type: "Control",
    move4name: "Primordial Eclipse",
    move4type: "Ultimate",
    move1desc: "Tear into the enemy with void energy.",
    move2desc: "Shroud the enemy in darkness, weakening their Power.",
    move3desc: "Silence the enemy's abilities, decreasing their Attack.",
    move4desc: "Unleash overwhelming primordial darkness.",

    move1use(attacker, defender) {
      const damage =
        damageDealt(attacker, defender, 1.0) + attacker.stats.power;
      defender.stats.currenthp = Math.max(0, defender.stats.currenthp - damage);

      return { damage: damage };
    },
    move2use(attacker, defender) {
      defender.stats.power = Math.max(0, defender.stats.power - 20);

      return { power: defender.stats.power };
    },
    move3use(attacker, defender) {
      defender.stats.attack = Math.max(0, defender.stats.attack - 30);

      return { attack: defender.stats.attack };
    },
    move4use(attacker, defender) {
      const damage = damageDealt(attacker, defender, 3.0);
      defender.stats.currenthp = Math.max(0, defender.stats.currenthp - damage);

      return { damage };
    },
    ability1name: CUSTOM_STATS["Nyx"].domainMastery,
    ability1desc: CUSTOM_STATS["Nyx"].masteryDesc,
    ability2name: CUSTOM_STATS["Nyx"].specialTrait,
    ability2desc: CUSTOM_STATS["Nyx"].traitDesc,
    ability1use(character) {
      character.stats.speed *= 0.2;
      character.stats.attack *= 0.3;
    },
    ability2use(character) {
      character.stats.power *= 2;
    },
  },
  Hypnos: {
    move1name: "Drowsy Tap",
    move1type: "Damage",
    move2name: "Sopor Mist",
    move2type: "Control",
    move3name: "Dreamy Veil",
    move3type: "Buff",
    move4name: "Endless Slumber",
    move4type: "Ultimate",
    move1desc: "A light attack that induces drowsiness.",
    move2desc:
      "Lull the enemy into sleep, lowering their Speed and Defense in the process.",
    move3desc: "Reduce incoming damage through calming dreams.",
    move4desc: "Force the enemy into an endless slumber.",

    move1use(attacker, defender) {
      const damage =
        damageDealt(attacker, defender, 0.7) + attacker.stats.power;
      defender.stats.currenthp = Math.max(0, defender.stats.currenthp - damage);

      return { damage: damage };
    },
    move2use(attacker, defender) {
      defender.stats.speed = Math.max(0, defender.stats.speed - 40);
      defender.stats.defense = Math.max(0, defender.stats.defense - 40);

      return { speed: defender.stats.speed };
    },
    move3use(attacker, defender) {
      attacker.stats.defense += 25;

      return { defense: attacker.stats.defense };
    },
    move4use(attacker, defender) {
      defender.stats.speed = 0;
      defender.stats.defense = 0;

      return { speed: defender.stats.speed, defense: defender.stats.defense };
    },
    ability1name: CUSTOM_STATS["Hypnos"].domainMastery,
    ability1desc: CUSTOM_STATS["Hypnos"].masteryDesc,
    ability2name: CUSTOM_STATS["Hypnos"].specialTrait,
    ability2desc: CUSTOM_STATS["Hypnos"].traitDesc,
    ability1use(character, opponent) {
      opponent.stats.speed = Math.max(0, opponent.stats.speed / 0.3);
      opponent.stats.defense = Math.max(0, opponent.stats.defense / 0.4);
    },
    ability2use(character, opponent) {
      opponent.stats.attack = Math.max(0, opponent.stats.attack / 0.2);
    },
  },
  Thanatos: {
    move1name: "Reaper's Slice",
    move1type: "Damage",
    move2name: "Final Omen",
    move2type: "Debuff",
    move3name: "Soul Sever",
    move3type: "Power",
    move4name: "Certain Death",
    move4type: "Ultimate",
    move1desc: "Slice the enemy with deathly precision.",
    move2desc:
      "Mark the enemy for inevitable death, reducing their Resilience.",
    move3desc: "Deal devastating true damage.",
    move4desc: "Instantly execute a weakened enemy.",

    move1use(attacker, defender) {
      const damage =
        damageDealt(attacker, defender, 1.4) + attacker.stats.power;
      defender.stats.currenthp = Math.max(0, defender.stats.currenthp - damage);

      return { damage: damage };
    },
    move2use(attacker, defender) {
      defender.stats.resilience = Math.max(0, defender.stats.resilience - 30);

      return { resilience: defender.stats.resilience };
    },
    move3use(attacker, defender) {
      const damage = Math.floor(attacker.stats.power * 1.5);
      defender.stats.currenthp = Math.max(0, defender.stats.currenthp - damage);

      return { damage: damage };
    },
    move4use(attacker, defender) {
      if (defender.stats.currenthp <= defender.maxhp * 0.3) {
        defender.stats.currenthp = 0;
        return { execution: true };
      }
      return { failed: true };
    },
    ability1name: CUSTOM_STATS["Thanatos"].domainMastery,
    ability1desc: CUSTOM_STATS["Thanatos"].masteryDesc,
    ability2name: CUSTOM_STATS["Thanatos"].specialTrait,
    ability2desc: CUSTOM_STATS["Thanatos"].traitDesc,
    ability1use(character) {
      character.stats.power += 30;
    },
    ability2use(character) {
      character.stats.attack += 30;
    },
  },
  Morpheus: {
    move1name: "Lucid Cut",
    move1type: "Damage",
    move2name: "Waking Nightmare",
    move2type: "Debuff",
    move3name: "Illusion Drift",
    move3type: "Control",
    move4name: "Dreamlord Dominion",
    move4type: "Ultimate",
    move1desc: "Slice through the enemy's dreams.",
    move2desc: "Distort perception and lower Attack.",
    move3desc: "Evade the next incoming attack by increasing your Speed.",
    move4desc: "Assert absolute control over the dreamscape.",

    move1use(attacker, defender) {
      const damage =
        damageDealt(attacker, defender, 1.0) + attacker.stats.power;
      defender.stats.currenthp = Math.max(0, defender.stats.currenthp - damage);

      return { damage: damage };
    },
    move2use(attacker, defender) {
      defender.stats.attack = Math.max(0, defender.stats.attack - 20);

      return { attack: defender.stats.attack };
    },
    move3use(attacker, defender) {
      attacker.stats.speed += 20;

      return { speed: attacker.stats.speed };
    },
    move4use(attacker, defender) {
      const damage =
        damageDealt(attacker, defender, 2.2) + attacker.stats.speed;
      defender.stats.currenthp = Math.max(0, defender.stats.currenthp - damage);

      return { damage: damage };
    },
    ability1name: CUSTOM_STATS["Morpheus"].domainMastery,
    ability1desc: CUSTOM_STATS["Morpheus"].masteryDesc,
    ability2name: CUSTOM_STATS["Morpheus"].specialTrait,
    ability2desc: CUSTOM_STATS["Morpheus"].traitDesc,
    ability1use(character, opponent) {
      opponent.stats.attack = Math.max(0, opponent.stats.attack - 20);
    },
    ability2use(character) {
      character.stats.speed += character.stats.speed * 0.15;
    },
  },
  Asclepius: {
    move1name: "Scalpel Strike",
    move1type: "Damage",
    move2name: "Restorative Touch",
    move2type: "Heal",
    move3name: "Purifying Salve",
    move3type: "Buff",
    move4name: "Miracle Intervention",
    move4type: "Ultimate",
    move1desc: "A careful, precise strike.",
    move2desc: "Restore a large amount of health.",
    move3desc: "Cleanse negative effects by boosting your stats.",
    move4desc: "Increase your own vitality.",

    move1use(attacker, defender) {
      const damage =
        damageDealt(attacker, defender, 0.8) + attacker.stats.attack;
      defender.stats.currenthp = Math.max(0, defender.stats.currenthp - damage);

      return { damage: damage };
    },
    move2use(attacker, defender) {
      const health = heal(attacker, 2);
      attacker.stats.currenthp = Math.min(
        attacker.maxhp,
        attacker.stats.currenthp + health
      );
      return { health: health };
    },
    move3use(attacker, defender) {
      attacker.stats.resilience += 15;
      attacker.stats.speed += 15;
      attacker.stats.attack += 15;
      attacker.stats.defense += 15;
      attacker.stats.power += 15;

      return { allBuffs: true };
    },
    move4use(attacker, defender) {
      attacker.stats.currenthp = Math.floor(attacker.maxhp * 0.8);
      return { revived: true };
    },
    ability1name: CUSTOM_STATS["Asclepius"].domainMastery,
    ability1desc: CUSTOM_STATS["Asclepius"].masteryDesc,
    ability2name: CUSTOM_STATS["Asclepius"].specialTrait,
    ability2desc: CUSTOM_STATS["Asclepius"].traitDesc,
    ability1use(character) {
      character.stats.power += character.stats.power * 0.3;
    },
    ability2use(character) {
      character.stats.currenthp = Math.min(
        character.maxhp,
        character.stats.currenthp + 300
      );
    },
  },
};

function damageDealt(attacker, defender, multiplier) {
  const attack = attacker.stats.attack;
  const defense = defender.stats.defense;

  const baseDamage = Math.max(5, attack - defense * 0.6);
  const damage = Math.floor(baseDamage * multiplier);

  return damage;
}

function heal(character, multiplier) {
  const power = character.stats.power;

  const healing = Math.floor(power * multiplier);

  return healing;
}

function statusApplied(type, stat, amount, turns) {
  return { type, stat, amount, turns };
}
