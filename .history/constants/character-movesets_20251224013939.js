...
export const CHARACTER_MOVES = {
  Zeus: {
    ...
    move1use(attacker, defender) {
      ...
      defender.stats.currenthp = defender.stats.currenthp - damage;
  ...
  Athena: {
    ...
    move1use(attacker, defender) {
      const damage =
        ...
        Math.floor(CUSTOM_STATS[attacker.name].defense * 0.3);
    ...
    move4use(attacker, defender) {
      ...
      const damage =
        damageDealt(attacker, defender, 1.0) *
        Math.floor(CUSTOM_STATS[defender].stats.attack);
  ...
  Apollo: {
    ...
    move3type: "Control",
    move3desc: "Chance to blind the enemy.",
...
function damageDealt(attacker, defender, multiplier) {
  const attack = CUSTOM_STATS[attacker.name].attack;
  const defense = CUSTOM_STATS[defender.name].defense;
...
function heal(character, multiplier) {
  const power = CUSTOM_STATS[character].power;
